...
exports.init = function (config, services) {
            ...
            function success (build) {
                commit.run('test').on('exit', function (code) {
                    ...
                    service.update(build.id, build).then(
                        function success () {
                            console.log('INFO: ' + Date.now() + ': Updated build: ' + JSON.stringify(build));
                        },
                        function failure () {
                            // TODO: Implement logging.
                            console.log('ERROR: ' + Date.now() + ': Failed to save build ' + build.commit + '@' + build.repo);                            
                        }
                    );
                });
            },
    ...
    http.createServer(ci.handle).listen(config.server.ports.integrator);
};